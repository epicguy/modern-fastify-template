...
class monitor {
  constructor(db) {
    ...
    this.schema = {
      ...
      q_last100job: { result: 1, date: 1, job_name: 1, err: 3, uuid: 1 },
      q_last100jobWork: { result: 1, date: 1, job_name: 1, err: 3, uuid: 1, did_work: 1 },
  ...
  async write_debug(ctx, row) {
    const f = 'db:monitor:write_debug:';
    //ctx.log(f, {row})
  ...
  _time_query(ctx, moment_now, last_secs, epoch_secs, duration_secs) {
    const f = 'db:monitor:_time_query:';
    ctx.log(f, { moment_now, last_secs, epoch_secs, duration_secs });

    if (duration_secs === 'epoch' && last_secs < 32 * 24 * 60 * 60) {
      // Max 1 month; If NaN, is false
    ...
    if (duration_secs === 'epoch' && epoch_secs < 32 * 24 * 60 * 60) {
      // Max 1 month; If NaN, is false
    ...
    if (['last-hour', 'epoch'].indexOf(duration_secs) > -1) {
      // Is default for epoch
  ...
  _projection_query(projection) {
    ...
    Object.keys(projection).forEach((key) => {
      if (key === 'date') sql_projection.push(`to_timestamp((log->>'start')::bigint/1000) as date`);
      else if (projection[key] === 1) sql_projection.push(`log->>'${key}' as ${key}`);
      else if (projection[key] === 2) sql_projection.push(key);
      else if (projection[key] === 3) sql_projection.push(`log->'${key}' as ${key}`);
      else if (projection[key] === 4) sql_projection.push(`log->'err'->>'${key}' as err_${key}`);
    });
  ...
  async q_last100jobWork(ctx, limit = 100) {
    const sql = `
			...
			FROM ${this.table}
			WHERE log ? 'job_name' AND log->>'did_work' != 'false' AND log->>'did_work' != 'add-job'
			ORDER BY id DESC
			LIMIT $1
		`;
    return await this.db.sqlQuery(ctx, sql, [limit], true);
  ...
  async q_deadlocks(ctx, moment_now, last_secs, epoch_secs, time_window, limit = 100) {
    const sql = `
			...
			FROM ${this.table}
			WHERE log->'err'->>'code' = ANY(ARRAY[ '40001','ER_LOCK_DEADLOCK']) AND ${this._time_query(ctx, moment_now, last_secs, epoch_secs, time_window)}
			ORDER BY id DESC
			LIMIT $1
		`;
    return await this.db.sqlQuery(ctx, sql, [limit], true);
  ...
  async q_dailyErrors(ctx, moment_now, last_secs, epoch_secs, time_window) {
    ...
    // FOR NOW WE'LL USE did_work==='run-job' meaning the job-wrapper caught a thrown error
    // NOT( COALESCE((log->>'statusCode')::integer,0) = ANY(ARRAY[ 200, 400, 401, 403, 404 ]))
    const sql = `
			SELECT count(*) as count, ${this._projection_query(this.schema.q_dailyErrors)}, max(log->>'uuid') as muuid
			FROM ${this.table}
			WHERE
				${this._time_query(ctx, moment_now, last_secs, epoch_secs, time_window)}
				AND CASE WHEN log->>'statusCode' IS NULL THEN
				  log->>'did_work' = 'run-job'
				ELSE
				  NOT( COALESCE((log->>'statusCode')::integer,0) = ANY(ARRAY[ 200 ]))
				END
			GROUP BY statusCode, route, verb, job_name, did_work, err_name, err_code, err_message
			ORDER BY count DESC
		`;
  ...
  async getDebug(ctx, uuid) {
    const sql = `
			SELECT id, log
			FROM ${this.table_deep}
			WHERE log->>'uuid' = ?
		`;
    return await this.db.sqlQuery(ctx, sql, [uuid]);
  }
}
