// Use:
// {jsonReplacer}= require './lib/json'
// JSON.stringify(the_object, _jsonReplacer(true));
...
var _exposeErrorProperties = function (error, show_stack = false) {
  ...
  if (show_stack === true && error.stack) {
    copy.stack = error.stack;
  }
  return copy;
};
var _jsonReplacer = function (show_stack) {
  return function (key, value) {
    if (value instanceof Error) {
      return _exposeErrorProperties(value, show_stack || key === 'cause');
    }
    return value;
  };
};

exports.exposeErrorProperties = _exposeErrorProperties;
exports.jsonReplacer = _jsonReplacer; // Use like: JSON.stringify(the_object, _jsonReplacer(true));
